import { ODEJ_KNOWLEDGE_BASE } from "./odej-knowledge";
...
function buildSystemPrompt(locale: string): string {
  ...
  return `أنت مساعد رسمي لمنصة ODG Algeria (ديوان مؤسسات الشباب).
${langHint}

القواعد الصارمة:
1. أجب فقط عن: ODG/ODEJ، المؤسسات الشبانية، الأنشطة، التكوين، التسجيل على المنصة، خلية الإصغاء بشكل عام، ديوان الشباب، الشراكات، وأسئلة الشباب المتعلقة بهذه الخدمات.
...
export async function handleChatPost(
  ...
  if (!apiKey) {
    return {
      ...
      body: {
        error:
          "Chat API not configured. Set GROQ_API_KEY in Vercel env or .env.local",
      },
    };
  }
  ...
  try {
    ...
    if (isOffTopic(lastUser.content)) {
      return {
        ...
        body: {
          message:
            "عذراً، أنا مساعد ODG وأجيب فقط عن منصة ديوان مؤسسات الشباب والخدمات الموجهة للشباب. يمكنك السؤال عن التسجيل، الأنشطة، التكوين، أو المؤسسات.",
          offTopic: true,
        },
      };
    }
